...
import { gad7Questions, gad7RatingOptions, gad7ScoringRules } from "../data/questions";
...
export default function Report() {
  ...
  const stored = sessionStorage.getItem("gad7Report");
  ...
  const totalScore = gad7Questions.reduce((sum, q) => {
  ...
  const getSeverity = (score) => {
    for (const rule of gad7ScoringRules.severity) {
  ...
  const downloadPDF = () => {
    html2canvas(reportRef.current, { scale: 2 }).then((canvas) => {
      ...
      pdf.save(`GAD7_Report_${patient?.name || "Patient"}.pdf`);
    });
  };
    ...
    <Container className="py-4">
      ...
      <div ref={reportRef} className="report-print">
        <div className="text-center mb-3">
          <h1 className="mb-1">GAD-7 Anxiety Assessment Report</h1>
          <small className="small-muted">
            GAD-7 is presented as an aid to healthcare professionals only and is not a substitute for clinical judgment.
          </small>
        </div>
        ...
        <Card className="mt-4">
          <Card.Body>
            ...
            <h5>Anxiety Severity: {severityLabel}</h5>
          </Card.Body>
        </Card>
      </div>
    </Container>
